...
console.log(process.env.APP_SERVER_MONGODB_URI)

export const configDB = async () => {
    try {
//        dotenv.config()
        //await mongoose.connect(varsConfig.APP_SERVER_MONGODB_URI);
        await mongoose.connect(varsConfig.ADRESS_MONGO as string);
        //const db = await mongoose.connect(process.env.ADRESS_MONGO);
